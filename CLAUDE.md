...
- Dark, iOS-flavored ops-console UI; mobile-first responsive layout (safe-area insets,
  single-column → grid at `lg`).
- **Interactive elements always get a hover state and a slight press state** — a clear `hover:`
  treatment plus a slight shrink on press (`active:scale-[0.92]`–`active:scale-[0.97]` with
  `transition-transform duration-150 ease-out`). This is the house style for the whole console.
- **Navigation lives in one place: `src/lib/nav.ts`** (`NAV_ITEMS` + `isActive`). The desktop top
  bar (`NavBar`) and the iOS-style bottom tab bar (`BottomNav`, shown on every screen size) both
  read from it — add new sections there, not in the components.
...
- The app is an installable PWA. The manifest is generated by `src/app/manifest.ts`; the app-shell
  service worker lives at `public/sw.js` and is registered (production only) by
  `src/components/ServiceWorker.tsx`.
- App icons under `public/icons/` are produced by `scripts/gen-icons.mjs` (renders the brand mark
  with Playwright's Chromium) — re-run it if the mark changes; don't hand-edit the PNGs.
- The `/download` page detects the visitor's device (`src/lib/device.ts`) and shows tailored install
  guidance plus a direct-install button where the browser fires `beforeinstallprompt`.
